...
function eslint() {
  ...
  const ignorePath = `${GITHUB_WORKSPACE}/.eslintignore`

  console.log("Start eslint engine", { ignorePath })
  const cli = new eslint.CLIEngine({ ignorePath })

  console.log("Start executeOnFiles")
  const report = cli.executeOnFiles([GITHUB_WORKSPACE || '.'])
  console.log("Finish executeOnFiles")
  ...
  console.log("Finish annotations")
...
async function updateCheck(id, conclusion, {title, summary, annotations}) {
  const output = {
    title,
    summary,
    // Github only allows max 50 annotations
